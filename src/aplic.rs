...
impl Aplic {
    ...
    pub fn set_domaincfg(&mut self, ie: InterruptEnable, dm: DeliveryMode, be: Endian) {
        let mut domaincfg: u32 = 0;
        if ie == InterruptEnable::Enabled {
            domaincfg |= 1 << 8;
        }
        if dm == DeliveryMode::MSIDeliveryMode {
            domaincfg |= 1 << 2;
        }
    ...
    ///   hhxw, and lhxw) as expected in the `mmsiaddrcfgh` register
    ///
    /// # Safety
    ///
    ...
    pub unsafe fn set_mmsiaddrcfg(&mut self, msi_addr: usize, mmsiaddrcfgh: u32) {
        ...
        self.mmsiaddrcfgh
            .write((msi_addr >> 44) as u32 | mmsiaddrcfgh);
    }
    ...
    ///   configuration specifying the MSI target addresses (hhxs, lhxs,
    ///   hhxw, and lhxw) as expected in the `smsiaddrcfgh` register
    ///
    /// # Safety
    ///
    /// This writes the memory address to allow hardware direct access
    ...
    pub unsafe fn set_smsiaddrcfg(&mut self, msi_addr: usize, smsiaddrcfgh: u32) {
        ...
        self.smsiaddrcfgh
            .write((msi_addr >> 44) as u32 | smsiaddrcfgh);
    }
